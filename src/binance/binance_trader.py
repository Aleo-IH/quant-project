...
class BinanceTrader:
    ...
    def __init__(self, testnet: bool = True):
        ...
        try:
            ...
            logger.error(
                f"ClientError: {e.error_message} (Code: {e.error_code}, Status: {e.status_code})"
            )
            sys.exit(1)
        except ServerError as e:
            logger.error(
                f"ServerError: {e.error_message} (Code: {e.error_code}, Status: {e.status_code})"
            )
            sys.exit(1)
    ...
    def get_candlestick_data(
        ...
        try:
            if isinstance(symbols, list):
                ...
                if data_frames:
                    df = pd.concat(
                        [df.set_index("Open Time") for _, df in data_frames],

...
from src.visualization.multioutput_multiclass import plot_cross_validation_metrics, plot_bin_precision
...
pipeline = Pipeline([
    ("preprocessor", ColumnTransformer(transformers=[
        ...
        ("discretizer", KBinsDiscretizer(n_bins=num_bins, encode='ordinal', strategy='quantile'), [col for col in df if re.search(r"Return", col)])
        ],
    remainder="passthrough"
    )),
    ("model", ClassifierChain(RandomForestClassifier(n_estimators= 100, n_jobs= -1, criterion ="log_loss")))
])

...
def plot_cross_validation_metrics(cv_results):
    ...
    for y_true, y_pred in cv_results:
        ...
        for i in range(y_true.shape[1]):
            ...
            fold_jaccard.append(
                jaccard_score(y_true.iloc[:, i], y_pred.iloc[:, i], average="micro")
            )
...
def plot_bin_precision(cv_result, n_bins=5):
    ...
    for y_test, y_pred in cv_result:
        for col in y_test.columns:
            for bin_num in range(n_bins):
                ...
                if (
                    pred_mask.sum() > 0
                ):  # Only calculate if model made predictions for this bin
                    ...
                    bin_precision[bin_num][col].append(
                        0
                    )  # No predictions made for this bin
    ...
    plt.title(
        "Precision by Predicted Bin (When model predicts a bin, how often is it correct?)"
    )
    ...
    for col in bin_precision[0].keys():
        ...
        for bin_num in range(n_bins):
            ...
            print(
                f"When predicted Bin {bin_num} - Precision: {mean_precision:.4f} ± {std_precision:.4f}"
            )

...
def test_fee_applied(trending_close_df):
    ...
    env = CryptoTradingEnv(
        trending_close_df, cols, transaction_fee=0.01, trade_threshold=0.0
    )
    env.reset()
    # Achat plein -> des frais doivent diminuer la balance vs sans frais
    env_no_fee = CryptoTradingEnv(
        trending_close_df, cols, transaction_fee=0.0, trade_threshold=0.0
    )
    env_no_fee.reset()
    env.step(4)
    env_no_fee.step(4)
